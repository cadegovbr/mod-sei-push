<?
class PUSHCriptografia {
	
	private static $KEY = 'c@d3s3mp@p3l';
	
	public static function criptografa($texto){
		try {
			return strtr(base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5(PUSHCriptografia::getChaveCriptografia()), serialize($texto), MCRYPT_MODE_CBC, md5(md5(PUSHCriptografia::getChaveCriptografia())))), '+/=', '-_,');
		} 
		catch (Exception $e) {
	...
	public static function descriptografa($texto) {
		try {
			return unserialize(rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5(PUSHCriptografia::getChaveCriptografia()), base64_decode(strtr($texto, '-_,', '+/=')), MCRYPT_MODE_CBC, md5(md5(PUSHCriptografia::getChaveCriptografia()))), "\0"));
		} 
		catch(Exception $e) {
			throw new InfraException('Erro validando link externo.', $e);
		}
	...
	public static function criptografaSiteCade($texto) {
		...
		$instrucao = 'java -jar ' . $caminho . ' \'criptografa\' \'' . $texto . '\' \'' . PUSHCriptografia::$KEY . '\' ';
	...
	public static function descriptografaSiteCade($texto) {
		$caminho = dirname(__FILE__) . '/criptografia/mascaraArgumentos.jar';
		$instrucao = 'java -jar ' . $caminho . ' \'descriptografa\' \'' . $texto . '\' \'' . PUSHCriptografia::$KEY . '\' ';
	...
	public static function descriptografaArgumentos($parametro) {
		...
		$parametrosDescriptografados = PUSHCriptografia::descriptografa($parametrosCriptografados);
